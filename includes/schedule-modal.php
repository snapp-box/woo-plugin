...
class SnappBoxScheduleModal {
    ...
    public function snappb_render_modal_html() {
        $schedule = $this->snappb_get_schedule();

        $days = $this->allowed_days;
        foreach ( $days as $d ) {
            if ( ! isset( $schedule[ $d ] ) || ! \is_array( $schedule[ $d ] ) ) {
                $schedule[ $d ] = [];
            }
        }
        ?>
        <div class="snappbox-modal-content snappbox-date-content none-class" id="dates">
            ...
            <h2><?php \esc_html_e( 'Select Day and Time', 'snappbox' ); ?></h2>

            <div class="selection-wrapper clearfix">
                <div class="snappbox-day-selector">
                    <p><?php \esc_html_e( 'Select Day', 'snappbox' ); ?></p>
                    <select id="snappbox-day" class="snappbox-day-selector">
                        <?php
                        $day_labels = [
                            'saturday'  => \esc_html_x( 'Saturday',  'weekday name', 'snappbox' ),
                            'sunday'    => \esc_html_x( 'Sunday',    'weekday name', 'snappbox' ),
                            'monday'    => \esc_html_x( 'Monday',    'weekday name', 'snappbox' ),
                            'tuesday'   => \esc_html_x( 'Tuesday',   'weekday name', 'snappbox' ),
                            'wednesday' => \esc_html_x( 'Wednesday', 'weekday name', 'snappbox' ),
                            'thursday'  => \esc_html_x( 'Thursday',  'weekday name', 'snappbox' ),
                            'friday'    => \esc_html_x( 'Friday',    'weekday name', 'snappbox' ),
                        ];
                ...
                <div>
                    <p><?php \esc_html_e( 'Add / Edit Time Slots', 'snappbox' ); ?></p>
                        ...
                        <?php foreach ( $schedule as $day => $slots ) : ?>
                            <div class="snappbox-day-slots" data-day="<?php echo \esc_attr( $day ); ?>" style="display:none;">
                                <?php foreach ( $slots as $slot ) : ?>
                                    <div class="time-slot">
                                        <input type="time" class="start" value="<?php echo \esc_attr( isset( $slot['start'] ) ? $slot['start'] : '' ); ?>" />
                                        <input type="time" class="end"   value="<?php echo \esc_attr( isset( $slot['end'] )   ? $slot['end']   : '' ); ?>" />
                                        <button type="button" class="remove-slot button" aria-label="<?php echo \esc_attr__( 'Remove slot', 'snappbox' ); ?>">
                                            <svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 48 48" width="16" height="16" aria-hidden="true"><path fill="#f44336" d="M44,24c0,11.045-8.955,20-20,20S4,35.045,4,24S12.955,4,24,4S44,12.955,44,24z"/><path fill="#fff" d="M29.656,15.516l2.828,2.828l-14.14,14.14l-2.828-2.828L29.656,15.516z"/><path fill="#fff" d="M32.484,29.656l-2.828,2.828l-14.14-14.14l2.828-2.828L32.484,29.656z"/></svg>
                                        </button>
                                    </div>
                                <?php endforeach; ?>
                            </div>
    ...
    public function snappb_save_schedule() {
        ...
        if ( \is_array( $slots_input ) ) {
            $raw_slots = \map_deep( $slots_input, 'sanitize_text_field' );
        } elseif ( \is_string( $slots_input ) ) {
            $decoded = \json_decode( $slots_input, true );
            if ( \json_last_error() === JSON_ERROR_NONE && \is_array( $decoded ) ) {
                $raw_slots = \map_deep( $decoded, 'sanitize_text_field' );
            } else {
                $raw_slots = [];
            }
        }
        ...
        \wp_send_json_success( [
            'message' => \__( 'Schedule saved successfully!', 'snappbox' ),
            'data'    => $saved_data,
        ] );
    }
    ...
    public function snappb_get_schedule() {
        $opt = \get_option( 'snappbox_schedule', [] );
        return ( \is_array( $opt ) ) ? $opt : [];
    }

    public function snappb_get_day_slots( $day ) {
        $schedule = $this->snappb_get_schedule();
        return isset( $schedule[ $day ] ) && \is_array( $schedule[ $day ] ) ? $schedule[ $day ] : [];
    }
}
