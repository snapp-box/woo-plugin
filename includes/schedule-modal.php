...
class SnappBoxScheduleModal {
    ...
    public function save_schedule() {
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['nonce'] ), 'snappbox_schedule_nonce' ) ) {
        ...
        $raw_slots = isset( $_POST['slots'] ) ? wp_unslash( $_POST['slots'] ) : [];
        ...
        if ( empty( $day ) ) {
            wp_send_json_error( [ 'message' => __( 'Day is required.', 'sb-delivery' ) ], 400 );
        }
